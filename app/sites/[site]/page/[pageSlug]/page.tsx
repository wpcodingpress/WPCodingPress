...
export default function PageRoute() {
  ...
  useEffect(() => {
    async function fetchPage() {
      if (!site || !pageSlug) return;
      
      setLoading(true);
      setError("");
      
      try {
        const response = await fetch(`/api/sites/${site}/page/${pageSlug}`);
        if (!response.ok) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p>{locale === "bn" ? "লোড হচ্ছে..." : "Loading..."}</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p>{locale === "bn" ? "পেজ পাওয়া যায়নি" : "Page not found"}</p>
        </div>
      </div>
    );
  }
